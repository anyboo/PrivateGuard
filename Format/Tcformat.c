...
BOOL myVolTransform(const char* volumePath, unsigned __int64 size, void *hwndDlgArg )
{
	if (!FormatInit())
		return FALSE;
	...
	volParams->bDevice = 0;
	volParams->hiddenVol = 0;
	volParams->volumePath = (LPSTR)volumePath;
	volParams->size = size;
	...
	volParams->sparseFileSwitch = 61;
	volParams->quickFormat = 0;
	volParams->sectorSize = GetFormatSectorSize();
	volParams->realClusterSize = &realClusterSize;

...
BOOL CALLBACK TravelerDlgProc (HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		if (lw == IDC_CREATE)
		{
			...
			// Driver
			sprintf (srcPath, "%s\\crypt.sys", appDir);
			sprintf (dstPath, "%s\\TrueCrypt\\crypt.sys", dstDir);
			if (!TCCopyFile (srcPath, dstPath))
			{
				handleWin32Error (hwndDlg);
				goto stop;
			...
			// Driver x64
			sprintf (srcPath, "%s\\crypt-x64.sys", appDir);
			sprintf (dstPath, "%s\\TrueCrypt\\crypt-x64.sys", dstDir);
			if (!TCCopyFile (srcPath, dstPath))
			{
				handleWin32Error (hwndDlg);
				goto stop;

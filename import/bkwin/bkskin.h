...
public:
	...
	static void HorzExtendDraw(CDCHandle &dc, CGdiPlusImage &imgDraw, CRect &rcDraw, LONG lSkinLeft, int nSubImage = -1)
	{
		SIZE sizeSkin;
	...
	static void VertExtendDraw(CDCHandle &dc, CGdiPlusImage &imgDraw, CRect &rcDraw, LONG lSkinTop, int nSubImage = -1)
	{
		SIZE sizeSkin;

		imgDraw.GetImageSize(sizeSkin);

		if (sizeSkin.cy > rcDraw.Height())
			rcDraw.bottom = rcDraw.top + sizeSkin.cy;

		if (imgDraw.GetSubImageWidth() > 0)
		{
			sizeSkin.cy = imgDraw.GetSubImageWidth();
		}

		imgDraw.Draw(
			dc,
			rcDraw.left, rcDraw.top,
			sizeSkin.cx, lSkinTop, 
			0, 0, 
			sizeSkin.cx, lSkinTop,
			nSubImage
			);
		imgDraw.Draw(
			dc, 
			//rcDraw.right - sizeSkin.cx + lSkinLeft + 1, rcDraw.top, 
			rcDraw.left, rcDraw.bottom - sizeSkin.cy + lSkinTop + 1,
			//sizeSkin.cx - lSkinLeft - 1, sizeSkin.cy, 
			sizeSkin.cx, sizeSkin.cy - lSkinTop - 1,
			//lSkinLeft + 1, 0, 
			0, lSkinTop + 1,
			//sizeSkin.cx - lSkinLeft - 1, sizeSkin.cy, 
			sizeSkin.cx, sizeSkin.cy - lSkinTop - 1,
		...
		imgDraw.Draw(
			dc, 
			//rcDraw.left + lSkinLeft, rcDraw.top, 
			rcDraw.left, rcDraw.top + lSkinTop,
			//rcDraw.Width() - sizeSkin.cx + 1, sizeSkin.cy, 
			sizeSkin.cx, rcDraw.Height() - sizeSkin.cy + 1,
			//lSkinLeft, 0, 
			0, lSkinTop,
			sizeSkin.cx, 1, 
			nSubImage
	...
	static void FrameDraw(CDCHandle &dc, CGdiPlusImage &imgDraw, CRect &rcDraw, LONG lSkinLeft, LONG lSkinTop, COLORREF crBg, UINT uDrawPart = Frame_Part_All)
	{
		SIZE sizeSkin;
		CRect rcClient = rcDraw;
...
class CBkGdiplusImageSkin
	: public CGdiPlusImage
	, public CBkSkinBase
{
	BKOBJ_DECLARE_CLASS_NAME(CBkGdiplusImageSkin, "gdiplusimglst")
...
public:
	...
	virtual void Draw(CDCHandle dc, CRect rcDraw, DWORD dwState)
	{
		CGdiPlusImage::Draw(dc, rcDraw.left, rcDraw.top, dwState);
	}
...
protected:
	...
	BKWIN_DECLARE_ATTRIBUTES_BEGIN()
		BKWIN_UINT_ATTRIBUTE("src", *(CGdiPlusImage *)(this), TRUE)
		BKWIN_INT_ATTRIBUTE("subwidth", m_lSubImageWidth, TRUE)
	BKWIN_DECLARE_ATTRIBUTES_END()
};

class CBkSkinGdiplusImageFrame : public CBkSkinBase
{
	BKOBJ_DECLARE_CLASS_NAME(CBkSkinGdiplusImageFrame, "gdiplusimgframe")

public:
	CBkSkinGdiplusImageFrame()
		: m_lSkinParamLeft(0)
		, m_lSkinParamTop(0)
		, m_uDrawPart(Frame_Part_All)
	{
...
class CBkSkinGdiplusImgHorzExtend : public CBkSkinBase
{
	BKOBJ_DECLARE_CLASS_NAME(CBkSkinGdiplusImgHorzExtend, "gdiplusimghorzex")
public:
	CBkSkinGdiplusImgHorzExtend()
		:m_lSkinParamLeft(0)
	{
	}
...
class CBkSkinGdiplusImgVertExtend : public CBkSkinBase
{
	BKOBJ_DECLARE_CLASS_NAME(CBkSkinGdiplusImgVertExtend, "gdiplusimgvertex")
public:
	CBkSkinGdiplusImgVertExtend()
		: m_lSkinParamTop(0)
	{
	}
	...
	{
		VertExtendDraw(
			dc, m_imgSkin, rcDraw, m_lSkinParamTop, 
			(-1 == dwState) ? -1 : IIF_STATE4(dwState, 0, 1, 2, 3)
			);
	}
...
//     }
    ...
    static CBkSkinBase* _CreateBkSkinByName(LPCSTR lpszName)
    {
		...
		pNewSkin = CBkGdiplusImageSkin::CheckAndNew(lpszName);
		if (pNewSkin)
			return pNewSkin;

		pNewSkin = CBkSkinGdiplusImgHorzExtend::CheckAndNew(lpszName);
		if (pNewSkin)
			return pNewSkin;

		pNewSkin = CBkSkinGdiplusImgVertExtend::CheckAndNew(lpszName);
		if (pNewSkin)
			return pNewSkin;

		pNewSkin = CBkSkinGdiplusImageFrame::CheckAndNew(lpszName);
		if (pNewSkin)
			return pNewSkin;
